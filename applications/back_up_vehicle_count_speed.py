...
def vehicle_count_and_speed(
    ...
    SMOOTH_FACTOR = 8    # Số mẫu để làm mịn
    ALPHA = 0.2         # Hệ số làm mịn
    ...
    while cap.isOpened():
        ...
        if tracks[0].boxes is not None and tracks[0].boxes.id is not None:
            ...
            for track_id, box in zip(track_ids, tracks[0].boxes.xyxy):
                ...
                # Tính tốc độ liên tục khi xe nằm trong vùng đo
                if region_top_y < center_y < region_bottom_y:
                    # Nếu xe vừa vào vùng đo, ghi nhận thời gian và vị trí ban đầu
                    if entry_times[track_id] is None:
                        entry_times[track_id] = timestamp
                        entry_positions[track_id] = center_x

                    # Tính tốc độ tại mỗi khung hình trong vùng đo
                    if entry_times[track_id] is not None:
                        current_time = timestamp
                        entry_time = entry_times[track_id]
                        if current_time > entry_time:
                            time_diff_sec = current_time - entry_time
                            ...
                            # Hiển thị tốc độ đã làm mịn
                            cv2.putText(frame, f"{filtered_speeds[track_id]:.1f} km/h", 
                                      (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 255, 255), 2)

                # Đặt lại entry_times khi xe rời khỏi vùng đo hoàn toàn
                if center_y <= region_top_y or center_y >= region_bottom_y:
                    entry_times[track_id] = None
                    entry_positions[track_id] = None
